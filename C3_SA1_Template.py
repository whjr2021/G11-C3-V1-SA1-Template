...
while carryOn:
    ...
    if event.type == pygame.KEYDOWN: # Checking if a key is pressed  
        ...
        # Check if key pressed is left key using "pygame.K_LEFT"
        if event.key ==              
            # Check if paddle's x-coordinate is greater than 0
            if paddle.x >  
                # Decrease paddle's x position by the value in 'paddlex'
                paddle.x 
